  ...
  return (
    <div className="absolute inset-0 z-0 px-5 w-full flex justify-center items-center bg-[linear-gradient(to_bottom_right,#FFFFFF_0%,#E0ECFF_25%,#EAE2FF_50%,#F8E8DB_75%,#FFFFFF_100%)]">
      <div className="w-full max-w-4xl mx-auto text-center">
        <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-forum text-[#0F172A] leading-snug sm:leading-tight md:leading-tight">
          Track Tuition Fees Easily. <br /> Send Automated Fee Reminders.
        </h1>

        <p className=" mt-2.5 sm:mt-4 text-sm sm:text-lg md:text-xl text-[#4B5563] max-w-2xl mx-auto">
          A smart fee tracker built for local tuition teachers & coaching
          centers
        </p>

        <div className="flex flex-row justify-center items-center gap-4 sm:gap-6 mt-5.5 sm:mt-8">
          <button className=" text-sm sm:text-base px-5 py-1.5 sm:px-6 sm:py-2 rounded-full bg-[#F97316] text-white font-medium shadow-md hover:bg-[#ea580c] transition-colors duration-200">
            Get Started
          </button>
          <button className=" text-sm sm:text-base px-5 py-1.5 sm:px-6 sm:py-2 rounded-full border border-[#6B7280] text-[#374151] hover:bg-[#f9fafb] transition-colors duration-200">
            How it Works
          </button>
        </div>
      </div>
